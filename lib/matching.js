...
export function getRenewalWindowState(renewalDate, referenceDate = new Date()) {
...
export function findBestClientMatch(targetName, candidateRows, threshold = MATCH_THRESHOLD) {
...
export function resolveDuplicateBillingRows(rows = []) {
...
export function reconcileClientExports({
  billingRows = [],
  projectRows = [],
  ...
  return {
    ...
    summary: {
      ...
      needsReview: matchedRows.filter((row) => row.matchStatus === 'missing_renewal_date').length + unmatchedProjects.length,
      billingOnly: orphanBillingRows.length,
...
export default matchingLibrary;
